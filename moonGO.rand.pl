...
getopts('iCcdhIbgNntBVvlM:m:a:T:D:S:p:P:H:G:s:f:',\%opts) || do { print "Invalid option, try 'moonGO.pl -h' for more information\n"; exit(1); };
my (@net_probs);
...
my $prob=$opts{p}||0.005; ## evalue significance threshold
     ...
     die("Need an annotated class file for mode 'i'\n") unless $annotations_file;
      bait:foreach my $bait (keys(%proteins)){
         $c++;
              my %seen_classes;
              my $is_cand=0;
	 my $kokoo=scalar keys (%moonlighting);
	 printf STDERR ("$c of $protnum : $kokoo\r") if $verbose;
...
print STDERR scalar keys(%moonlighting) .  " candidates found (before MT correction).\n" if $verbose;
if ($no_multi){
...
print STDERR scalar keys(%cands_kept) .  " candidates found (after MT correction).\n" if $verbose;
...
COMMAND-LINE OPTIONS:
    ...
    -p : minimum e-value threshold (def= 0.005)
