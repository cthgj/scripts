...
use strict;
my %opts;
getopts('epva:d:G:g:m:n:o:s:',\%opts);
...
if($opts{p}){
	...
	for (my $k=$i; $k<scalar(@GOs);$k++) {
	    ...
	    my $go1cnt=$interactions_gc{$go1}{$oo}||0;
	    my $go2cnt=$interactions_gc{$go2}{$oo} ||0;
	    if($go1cnt>0){
  ...
  c_loop($#GOs,$verbose,\@GOs,\%interactions_g,\%interactions_o,\%interactions_gl,\%interactions_gc,\%onto_count,\%{$go_ontos{ONTO}});
...
#include "uthash.h"
/* c_loop($#GOs,        $verbose,      \@GOs,   \%interactions_g,  \%interactions_o, \%interactions_gl,   \%interactions_gc,   \%onto_count, \%{$go_ontos{ONTO}});*/
void c_loop(int max, int verbose, SV *SVGOlist, SV *SVinter_gos, SV *SVinter_ontos, SV *SVinter_golist, SV *SVinter_gocount, SV *SVonto_count,SV *SVgo_ontos){
 ...
 HV *inter_gocount=(HV*)SvRV(SVinter_gocount);  /* get a ref to %interactions_gc */
 ...
 for(counter1=0;counter1<=max; counter1++){ 
   ...
   /***********/
	...
	if(hashref2 != NULL){
     ...
     /*******************************************************/
     int go2count = 0;
     SV **go2_rr= hv_fetch(inter_gocount, go2, strlen(go2),0);
     if(go2_rr != NULL){
       HV *go2_hh=(HV*)SvRV(*go2_rr);
       SV **go2_r = hv_fetch(go2_hh, oo, strlen(oo),0);
       if(go2_r != NULL){
	 go2count=SvIV(*go2_r);
       }
     }
