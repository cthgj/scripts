...
my $k=0;
while($n<scalar(@GOs)){
    my $gopair=get_pair($n,$k,@GOs);
    shift(@GOs);
    $n++;
    print STDERR "$n of $#GOs ($gopair)\r" if $verbose; 
    next if $gopair=~'GO:0008150' ; ## skip P root
    next if $gopair =~ 'GO:0005575' ; ## skip C root
    next if $gopair =~ 'GO:0003674' ; ## skip F root 
    my @b=sort {$b lt $a} (split("/_/",$gopair));
    my $pair = join("_",@b);
    $go1=$b[0];
    $go2=$b[1];
...
SV* get_pair(int n, int k, SV* go1, ...){
  //First, we need to begin our function with a "Inline_Stack_Vars" statement. This defines a few internal variables that we need to access the Stack. Now we can use "Inline_Stack_Items", which returns an integer containing the number of arguments passed to us from Perl.
  //NOTE: It is important to only use "Inline_Stack_" macros when there is an elllenipsis (...) in the argument list, or the function has a return type of void.
  Inline_Stack_Vars;
  SV* pair[100];
//  printf ("---------------------------\n");
  /* strcpy(pair,SvPV(Inline_Stack_Item(1),PL_na)); */
  //  for (i = 0; i < 2; i++) {
    strcpy(pair,SvPV(Inline_Stack_Item(n),PL_na)) ; // ,"_"SvPV(Inline_Stack_Item(k),PL_na));
    strcat(pair,"_");
    strcat(pair,SvPV(Inline_Stack_Item(k),PL_na));
 ...
 // printf("AA %c%c\n",pair[1],pair[2]);
  /* Inline_Stack_Reset; */
  /* for (i = 0; i < 2; i++) */
  /*   Inline_Stack_Push(pair[i]); */
  /* Inline_Stack_Done; */
  //strcpy(ret,pair)
    //printf("Pair : %s\n",pair);
  return newSVpvf(pair);
