...
############################################################################
    case ['B','unique_bridge','b','bridge'] {
	  ...
	  t2:for (my $iii=$ii; $iii<=$#TARGETS; $iii++) {
		  ...
		  ##########################################
		  # next t2 if it shares and class with bait #
		  ##########################################
		  map{
...
if($read_network_prob_file){
	...
	my $Inetwork_prob_file = $network_file . ".inter.prob";
	my $Anetwork_prob_file = $network_file . ".prob";
	if($check_both){
	    open(NET,">>$Anetwork_prob_file");
	    map{print NET}@net_probs;
	    close(NET);
	    system("cat $Anetwork_prob_file | sort | uniq > lililolo.$$; mv lililolo.$$ $Anetwork_prob_file");
	    open(iNET,">>$Inetwork_prob_file");
	    map{print iNET}@inet_probs;
	    close(iNET);
	    system("cat $Inetwork_prob_file | sort | uniq > lililolo.$$; mv lililolo.$$ $Inetwork_prob_file");
	}
	elsif ($check_interactome_probs) {
	    open(iNET,">>$Inetwork_prob_file");
	    map{print iNET}@inet_probs;
	    close(iNET);
	    system("cat $Inetwork_prob_file | sort | uniq > lililolo.$$; mv lililolo.$$ $Inetwork_prob_file");
...
sub parse_input_files_guess_values{
    ...
    $species=guess_species($network_file);
...
sub parse_gaf_file{    
	...
	debug("xGO: $name : " . keys(%{$proteins{$name}{GOs}}) . "\n"); 
    }
    close(A);
}
############################################################
...
sub interactome_probability{
	...
	debug "INTER_P : $gopair ($c): $inter_prob{$gopair} 1":
	    debug "INTER_P $gopair ($c): NOT DEFINED";
	...
	$inter_prob{$gopair}=1; 
	debug "$inter_prob{$gopair} (3)\n";
	return($inter_prob{$gopair});	
    }
    if($read_network_prob_file){
	my $network_prob_file = $network_file . ".inter.prob";
	...
	if (defined($inter_prob{$gopair})){debug("Prob :$inter_prob{$gopair}\n") }
	else {debug("Inter_Prob :$gopair NOT IN DB\n") unless defined($inter_prob{$gopair});} 
    }
    unless(defined($inter_prob{$gopair})){
	$inter_prob{$gopair}=1;
	push @inet_probs, "$gopair\t$inter_prob{$gopair}\n" if $read_network_prob_file;  
	return($inter_prob{$gopair});
    }
    ## @inet_probs will be used to create a prob file for this network
    push @inet_probs, "$gopair\t" . $inter_prob{$gopair} . "\n";   
    debug("INTER: $gopair :: $inter_prob{$gopair}");
...
sub gopair_probability{
    ...
    if($read_network_prob_file){
	my $network_prob_file = $network_file . ".prob";
...
COMMAND-LINE OPTIONS:
    ...
    -b : Check BOTH interactiome AND annotation probs.
    -c : Color output
    ...
    -n : Ignore annotations inferred from the class file. Useful when counting
	 ...
	 we are only interested in the direct/inherited annotations and not 
         the inferred ones.
    -N : Read/Create the network probability file (NETWORK_NAME.prob)
    -o : Gene Ontology:
