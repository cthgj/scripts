...
sub load_network {
	...
	foreach my $target (@targets){
	    ...
	    # if $local_seen{$bait}{$target}>1				    #
            #################################################################
	    if($p1 eq $target){
		if (exists($local_seen{$target}{$p1}) &&
		  $local_seen{$target}{$p1}>1){next}  
	    }
	    else{
		next if exists($local_seen{$target}{$p1});
	    }
	    for(my $i=0; $i<scalar(@w);$i++){
		for(my $k=$i; $k<scalar(@w);$k++){
		    # if($debug>1){print STDOUT "A \$prots{$p1}{ONTOs}{$w[$i]}:$prots{$p1}{ONTOs}{$w[$i]} \$prots{$target}{ONTOs}{$w[$k]}:$prots{$target}{ONTOs}{$w[$k]} ";}
		    my $oo=join("", sort {$b lt $a} ($w[$i],$w[$k]));;
		    if(defined($prots{$p1}{ONTOs}{$w[$i]}) && 
		       defined($prots{$target}{ONTOs}{$w[$k]})){
			$oo = join("", sort {$b lt $a} ($w[$i],$w[$k]));
			$go_counts{ONTOs}{$oo}++;
			$wanted_proteins{$oo}{$p1}=1;
		    }
		    else{$go_counts{ONTOs}{$oo}+=0}
		    if($debug>1){print STDOUT "\$go_counts{ONTOs}{$oo}:$go_counts{ONTOs}{$oo}\n";}
		}
	    }
	}
	foreach my $go (keys(%{$prots{$p1}{GOs}})){
	    foreach my $onto (keys(%want)){
		my $oo = join("", sort {$b lt $a} ($go_ontos{ONTO}{$go},$onto));
		$counts{$go}{$oo}+=&count_go_interactions($p1,$go,$debug,\%{$go_ontos{ONTO}}, $oo,\@targets, \%prot_annots, \%local_seen);
		## If this prot embodies an interaction between
		## both ontologies, we want it
		# if(exists($local_want{$oo})&& ## if this prot has interactions bridging both ontos
		#    $local_want{$oo}<2){
		#     $local_want{$oo}++;
		#     $wanted_proteins{$oo}{$p1}=1;
		# }
...
SV* c_loop_inter2(int max,int verbose, SV* SVontology, int debug, SV* go_ontos_Oref, SV* go_counts_Gref, SV* go_counts_Oref, SV* gos_hash_ref, SV* wanted_prots_ref, SV* inters_hash_ref, SV* GOsSV){
  ...
  for(counter1=0;counter1<max; counter1++){ 
	...
	for(cc=0;cc<=av_len(go1_protlist);cc++){
	  ...
	  if (debug>0)
	    printf("a1bbbbbbbbbbbbbbbWXSWXS :%s:%s :: %d\n",go1,go2,cc);
	}
	if (debug>0)
	  printf("6GGGGGGGGGGGGGGggggggg :%s:%s\n",go1,go2);
      }
      if (debug>0)
