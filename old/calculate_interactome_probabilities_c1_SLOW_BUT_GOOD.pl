...
if($opts{c}){
    c_loop($#GOs,$verbose,\@GOs,\%gos,\%wanted_prots,\%interactions,\%{$go_ontos{ONTO}}, \%{$go_counts{ONTOs}}, \%{$go_counts{GOs}});
...
if($opts{p}){
  for (my $i=0; $i<scalar(@GOs);$i++) {
    ...
    for (my $k=$i; $k<scalar(@GOs);$k++) {
      ...
      my $both=0;
      my %seen;
      foreach my $prot1 (@{$gos{$GOs[$i]}} ) {
	next unless defined($wanted_prots{$oo}{$prot1});
	foreach my $prot2 (@{$gos{$GOs[$k]}}) {
	  next unless defined($wanted_prots{$oo}{$prot2});
	  my @pp=sort{$b lt $a} ($prot1,$prot2);
	  if (defined($interactions{$pp[0]}{$pp[1]})){
	    $both++ unless exists($seen{$pp[0] . $pp[1]});
	    $seen{$pp[0] . $pp[1]}++;
	  }
	}
      }
...
sub parse_gaf{
  ...
  while (<$GAF>) {
	...
	push @{$prots{$prot}{GOList}}, $pp[$i] unless $seen{$prot}{$pp[$i]};
	$prots{$prot}{GOs}{$pp[$i]}++ unless $seen{$prot}{$pp[$i]};
	$seen{$prot}{$pp[$i]}++;
      }
    }
...
sub load_network {
  ...
  while (<$A>) {
    ...
    if ($mode==0) {
	  ...
	  $NONE++ unless defined($prots{$bait}{GOs});
      } 
      else {
	  next if $interactors{$bait}{$target}>1;
	  $NONE++ unless defined($prots{$target}{GOs});
	  $NONE++ unless defined($prots{$bait}{GOs});
      }
	...
	foreach my $o2 (keys(%want)) {
      ...
      ## this interaction
      my %k;
      my @redundant_gos=(keys(%{$prots{$bait}{GOs}}),keys(%{$prots{$target}{GOs}}));
  ...
  $mode==0 ? return(\%h) : return(\%go_stats,\%interactors,\%w);
}
############################################################
## get_name                                               ##
############################################################
...
#include "uthash.h"
 ...
 int c_loop(int max, int verbose, SV *SVGOlist, SV *SVgos, SV *SVwanted_prots, SV *SVinteractions, SV *SVgo_ontos, SV *SVonto_counts, SV *SVgo_counts){
  ...
  for(counter1=0;counter1<=max; counter1++){ 
	...
	/*********************************/
	/* Iterate the list of go2 prots */
	/*********************************/
	for(cc2=0;cc2<=av_len(go2_prot_list); cc2++){ 
	  /***************************************/
	  /* Get the name of the go1 prot	       */
	  /***************************************/
	  SV **go2_prot_ref=av_fetch(go2_prot_list,cc2,0);
	  SV *go2_prot;
	  if(go2_prot_ref != NULL){
	    go2_prot=*go2_prot_ref;
	  }	  
	  /*******************************************************/
	  /* Check if this protein is wanted in this onto        */
	  /*******************************************************/
	  if(onto_wanted_prots_ref != NULL &&
	     hv_exists_ent(onto_wanted_prots,go2_prot,0)==0){
	    continue;
	  }
	  /**************************************************************************/
          /* Protein interactions are stored alphabetically as $interactions{a}{b}. */
          /* We therefore need to sort the protein names in order to get the 	    */
          /* appropriate hash entry.						    */
          /**************************************************************************/
	  char prot1[20], prot2[20];
	  if(strcmp(SvPV(go1_prot,PL_na), SvPV(go2_prot,PL_na))<0){
	    strcpy(prot1,SvPV(go1_prot,PL_na));
	    strcpy(prot2,SvPV(go2_prot,PL_na));
	  }
	  else{
	    strcpy(prot2,SvPV(go1_prot,PL_na));
	    strcpy(prot1,SvPV(go2_prot,PL_na));
	  }
	  char hash_id[60];
	  ...
	  if (s){
	    ...
	    //if((strcmp(go1,go2)==0) || (strcmp(prot1,prot2)==0)){
	      //add to hash
	  ...
	  /***********************************/
	  SV** prot1_interhash_ref=hv_fetch( inters, prot1 , strlen(prot1), 0);
	  HV* prot1_interhash;
	  if( prot1_interhash_ref != NULL){
	    prot1_interhash=(HV*)SvRV(*prot1_interhash_ref);
	    /**********************************************/
