		...
		class1:foreach my $class1 (keys(%{$proteins{$target1}{CLASS}})){ 
		  ...
		  class2:foreach my $class2 (keys(%{$proteins{$target2}{CLASS}})){  
			...
			c2:foreach my $class2_annot (@uniqu2){	
			    ...
			    ## If we are checking BOTH annot and inter probs
			    if($check_both){
				my $Pi=&interactome_probability($class1_annot,$class2_annot);
				my $Pa=&gopair_probability($class1_annot,$class2_annot);	
				if($Pa<=$prob && $Pi<=$prob){
				    ## Make sure we keep the MOST specific LEAST probable of the interactions
				    ## in question
				    if(defined($moonlighting{$bait}{$class_name})){
   ...
   case ['c','difclass'] {
	...
	target:foreach my $target (keys(%{$proteins{$bait}{INTERACTORS}})){
	   ...
	   # my $is_cand=0;
			...
			## If we are checking BOTH annot and inter probs
			if($check_both){
			    my ($Pa,$Pi)=(&interactome_probability($bgo,$tgo),&gopair_probability($bgo,$tgo));	
			    if($Pa<=$prob && $Pi<=$prob){
				## Make sure we keep the MOST specific LEAST probable of the interactions
				## in question
	...
	c1:for(my $n=0; $n<=$#cls;$n++){
	  c2:for(my $k=$n+1; $k<=$#cls;$k++){
		...
		my $prec1=&term_precision($class1_annotation);
		  ...
		  ## If we are checking BOTH annot and inter probs
		  if($check_both){
		      my ($Pa,$Pi)=(&interactome_probability($class1_annotation,$class2_annotation),&gopair_probability($class1_annotation,$class2_annotation));	
		      if($Pa<=$prob && $Pi<=$prob){
			  my $a=$cls[$n] . "_" . $cls[$k];
...
sub calculate_percentages{
    ...
    if( $perc>=50){
       ...
       ## skip target if any of its gos are related to the mcgo
		## If we are checking BOTH annot and inter probs
		if($check_both){
		    my $Pi=&interactome_probability($mcgo,$tgo);
		    my $Pa=&gopair_probability($mcgo,$tgo);	
		    if($Pa>=$prob || $Pi>=$prob){
			$not_moonlighting{$bait}{$target}=1;
			next target;
		    }
