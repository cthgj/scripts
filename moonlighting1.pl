...
sub calculate_percentages{
    ...
    my @bait_gos=@{(shift)};
    my @target_gos=@{(shift)};
    my @target_names=@{(shift)};
    my %bait_hash=%{(shift)};
    my %target_hash=%{(shift)};
    my $target=$target_hash{NAME};
    my %gostats;
    my $koko=shift;
    my $pp=shift;
    my ($mc,$mcgo,$perc)=0;
## Calculate most common go stats for this bait
	...
	my ($io,$ll)=0;
	## foreach target go
	g:foreach my $g (keys(%gostats)){
	    $io++; ## count target gos
	    next if $g eq $mcgo;
	    next if $g eq "none";
	    ## Next if this go is related to the mcgo
	    if (exists($offspring{$g}{$mcgo}) ||
		exists($offspring{$mcgo}{$g}))
	    {next g}
	    ## Cycle through bait GOs
	    $ll=0;
	    for (my $i=0; $i<=$#bait_gos; $i++) {
		$ll++; ## count bait gos
		next if $bait_gos[$i] eq "none";
		## next target if the bait 
		#return if defined($gostats{$bait_gos[$i]});
	
		my $GOpair=$g . "xxx" . $bait_gos[$i];
		printf(STDERR "$bait <=> $target $GOpair ($koko of $pp), TGO:$io of " . scalar(keys(%gostats)) .", BGO:$ll of " . scalar(@bait_gos) . "\r") if $verbose;
		my $P=&gopair_probability($GOpair,0);
		if(($P>=$high_prob) && ($P<=$low_prob)){##fixme
		    $candidates{$bait}{$target}{STRING}="$bait($bait_gos[$i]: $bait_hash{'source'})\t<==>\t$target_names[$i]($g: $target_hash{'source'}) : \t$P";
		    $candidates{$bait}{$target}{P}=$P;
		    
		    
		    #print "$bait($bait_gos[$i])::$target_names[$i]($g) : $P\n";
