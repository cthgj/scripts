...
sub main{
	...
	#	  }
	  ...
	  &calculate_percentages(\%bait_info,$koko,$pp);
...
sub calculate_percentages{
    ...
    {
	foreach my $tgo (@{$hash{$target}{GOs}}){ ## tgo is the target's go
	    $gostats{$tgo}++;
	}
    }
	...
	g:foreach my $g (keys(%gostats)){
	    ...
	    $ll=0;
	    for (my $i=0; $i<=$#{$hash{GOs}}; $i++) {
		$ll++; ## count bait gos
		next if $hash{GOs}[$i] eq "none";
		## next target if the bait 
		#return if defined($gostats{$bait_gos[$i]});
	
		my $GOpair=$g . "xxx" . $hash{GOs}[$i];
		printf(STDERR "$bait <=> $target $GOpair ($koko of $pp), TGO:$io of " . scalar(keys(%gostats)) .", BGO:$ll of " . scalar(@{$hash{GOs}}) . "\r") if $verbose;
