...
#
# 
# $Id: SECISearch.pl,v 1.3 2003/07/02 10:37:30 cchapple Exp cchapple $
#
...
my $bdir    = "/usr/local/molbio/share/SECISearch/";
...
my %opts;
getopts('p:f:e:o:RHITPYOBShvtgsndcilx',\%opts) || do { print "Invalid option, try 'SECISearch -h' for more information\n"; exit(1); };
...
my %e2      = (pat_standard_I_II => -17, pat_non_standard_I_II => -17, pat_twilight_I_II => -17);
...
my $nolog   = $opts{x} || 0;    # do not create the secislog logfile

my $cmpltry = 1;
if($opts{c}) {$cmpltry = 0};# search conmplimentary strand by default
...
#   if ($opts{e}){$e1 = $opts{e};} else {$e1 = $e1{$patname} || -10}   Old energy for core
    if ($opts{e}){$e2 = $opts{e};} else {$e2 = $e2{$patname} || -16} 
...
#    &debug( "\$ is $");
 ...
 open (LOG, ">>secislog") unless $nolog;
...
### Old energy stuff ($upstemen = energy(3..$unit8)) <=$e1& 		
		...
		if ($start>$stop)
		{
		    print EN ">$seqname [$stop - $start] (SECIS on complementary strand) - Free Energy: $fullstructen\n$currentseq\n$currentstruct\n";
		}
		else
		{
		    print EN ">$seqname [$start - $stop] - Free Energy: $fullstructen\n$currentseq\n$currentstruct\n";
		}
...
sub gff_output
{
	...
	print  GFF "$realname\tSECISearch\tSECIS\t$stop\t$start\t$score\t$strand\t$frame\t$group\n" if $strand eq '-';
	close (GFF);
  }   
}
...
DESCRIPTION: 
    ...
    pattern is specified by the -p option, SECISearch uses its own default pattern.


USAGE:
	
    SECISearch [OPTIONS] -p [patscan pattern file] -f [FASTA input file]
