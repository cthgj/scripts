...
sub main{
	...
	target:foreach my $target (@{$interactions{$bait}}){
	  ...
	  thash:foreach my $h (@{$proteins{$target}{$subonto}}){
		  ...
		  foreach my $baitgo (@bait_gos){
			  ...
			  next if $GOpair =~/none/;
			  my $P=&gopair_probability($GOpair,0,$bait,$target);
	...
	my $koko=1;
	 bait:foreach my $bait (keys(%interactions)) {
	     ...
	     ## to allow indentification of the off one(s) out
	     my @targets=keys(%{$interactions{$bait}});
	     next bait if scalar(@targets)<3;
		...
		#     push @target_gos,"none";
		 #    push @target_names,$target; 
		     ...
		     push @target_names,$target;
              ...
              ## Calculate most common go stats for this bait
	     my %clusters=&cluster_gos(@target_gos);
	     next unless defined($clusters{UNCLUSTERED});
	     map{print "=====================================\naa $_ : @{$clusters{$_}}\n"}keys(%clusters);
	     #die();
	     print STDERR "\n################################################## finished $bait ##################################################\n" if $very_verbose;
	     map{
	     ...
	     my $perc=($mc/scalar(@target_gos))*100;
	     print "PP : $perc\n" if $perc>=50;
    ...
    # 	    else{
    # 		%bait_hash=%{$h};
    # 		next bait_hash if $bait_hash{"Qualifier"} eq "NOT";
    # 		my $bait_go=$bait_hash{"goID"};
    # 	      target:foreach my $target (@{$interactions{$bait}}){
    # 		  $ll++;
    # 		  next target if defined($skip_this{$target});
    # 		  &debug("\tTARGET : $target");
    # 		  printf(STDERR "$koko of $pp, $io of " . scalar(@{$proteins{$bait}{$subonto}}) .", $ll of " . scalar(@{$interactions{$bait}}) . " \r");
    ...
    # 			  %target_hash=%{$h};
    # 			  next thash if $target_hash{"Qualifier"} eq "NOT";
    # 			  ## Go to the next target protein if bait's go is related
    # 			  ## to one of the target's. And save the relationship to skip 
    # 			  ## this pair later.
    # 			  if (exists($offspring{$bait_hash{"goID"}}{$target_hash{"goID"}}) || 
    # 			      exists($offspring{$target_hash{"goID"}}{$bait_hash{"goID"}}) ||
    # 			      defined($related{$bait}{$target})){
    # 			      $related{$bait}{$target}=$related{$target}{$bait}=1; 
    # 			      next target;
    # 			  }
    # 			  my $GOpair= $bait_hash{"goID"}. "xxx" . $target_hash{"goID"};
    # 			  my $P;
    # 			  if(defined($seen_probs{$GOpair})){$P=$seen_probs{$GOpair}}
    # 			  else{
    # 			      $P=&gopair_probability($GOpair,0);
    # 			  }
    # 			  ## Save this probbility so we don't need to calculate it again
    # 			   $seen_probs{$GOpair}=$seen_probs{$target_hash{"goID"}. "xxx" .$bait_hash{"goID"} }=$P;
    # 			  if($P<=$low_prob){##fixme
    # 			      $candidates{$bait}{$target}{STRING}=$GOpair . "xxx" . $bait_hash{"source"} . "xxx" . $target_hash{"source"};
    # 			      $candidates{$bait}{$target}{P}=$P;
    # 			      next bait;
...
sub cluster_gos{
    my @GOs=@_;
    my (%clustered,%clusters);
    push @{$clusters{$GOs[0]}},$GOs[0];
    $clustered{$GOs[0]}++;
 #   for (my $n=0; $n<=$#GOs;$n++){
   for (my $n=0; $n<=$#GOs;$n++){
       print STDERR "|" if $very_verbose;
    	for (my $k=$n+1; $k<=$#GOs;$k++){
	    next if $GOs[$k] eq $GOs[$n];
	    next if defined($clustered{$GOs[$k]});
	    print STDERR "." if $very_verbose;

    	    my $P=&gopair_probability($GOs[$n] . "xxx" . $GOs[$k],0);
	    if ($P>$low_prob){
#	    	push @{$clusters{$GOs[$n]}},$GOs[$k]; 
	    	push @{$clusters{$GOs[$n]}},$GOs[$k]; 
		$clustered{$GOs[$k]}++;
	    }
    	}
   }
    map{push @{$clusters{UNLCUSTERED}},$_ unless defined $clustered{$_}}@GOs;
    print STDERR "\n" if $very_verbose; 
    return(%clusters);
...
sub gopair_probability{
    ...
    $go2=~/GO:((.....).+)$/;
    my ($b1,$b2)=($1,$2);
    if (-e "$network_file.prob"){##remember to change back
	print STDERR "$network_file.prob found, opening...\n" if $verbose;
	open(A,"$network_file.prob");
	while(<A>){
	    chomp;
	     if(( /$b1/) &&( /$a1/)){
	    	my @tt=split(/\t/);
	    	$prob{$gopair2}{"high"}=$prob{$gopair}{"high"}=$tt[1];
	    	$prob{$gopair2}{"low"}=$prob{$gopair}{"low"}=$tt[2];
	    }
	}
	close(A);
    }
	...
	$a1>$b1 ? ($file=$b2 . ".prob") : ($file=$a2 . ".prob")  ;
	    ...
	    else{die("bad stats filename: $stats_dir/$file:$gopair\n")}
	    while(<A>){
		chomp;
		if(( /$b1/) &&( /$a1/)){
		    my @tt=split(/\t/);
...
#		die("crap : $gopair/$gopair2: " . $prob{$gopair}{"high"} . ":". $prob{$gopair2}{"high"} ."\n") ;
		print STDERR "Probability problem, $gopair" . $prob{$gopair}{"high"} . ":". $prob{$gopair}{"low"} . ":::". $prob{$gopair2}{"high"} . ":". $prob{$gopair2}{"low"} . "\n";
