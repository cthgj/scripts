...
#    if($struc[6] =~ /^(\(+)(\.+)(\)+)/){
	...
	my ($c,$d,$e,$f) = ($1,$2,$3,$4);
	&writeX(length($c),8,\$c,1);
	&writeX(length($d),9,\$d,1);
	&writeX(length($e),8,\$e,1);
	&writeX(length($f),15,\$f);
