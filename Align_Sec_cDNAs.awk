...
# This program extracts potential SP alignments between cDNAs/ESTs
# at the protein level (tblastx output). We search all possible * in the cDNA 
# subject and check if they align to */C in the query. 
# This shell works for multiple * (necessary when translating cDNAs/ESTs)
...
BEGIN{
    ...
    SCOREMAX=10000;
    EVALUE=0.001;
    IDENTITY=100; # Value of 100 to remove identical matches (Queries are also subjects). But need
	              # to do an unmask blast to avoid lowering the identity value. 
...
# Many * may exist in the cDNA/EST
# We want to check all them
# and get, if any, the one
# that aligns to a * or a C
...
# Sec alignment (C-* or whatever we are interested)
     ...
     if ((sc != 0) && (bit==0) && (score_>SCOREMIN) && (score_<SCOREMAX) && (identities!=IDENTITY) && (expect+0<EVALUE) &&((substr(query,sc,1) ~ PAT))) 
	 {
	     ...
	     if (query_ID_last==query_ID_Sec && sbjct_ID_last==sbjct_ID_Sec && score_Sec==score_last && identities_Sec==identities_last && expect_Sec==expect_last && pos_start_query_Sec > pos_end_query_last) # check we are in the same HSP
		 {
		     ...
		     print query_ID_Sec, "("query_len" nt)";
		     print "\n";
		     print sbjct_ID_Sec;
		     print "\n";
...
# Special case when an *-* alignment follows another *-* in different HSPs
	     ...
	     else if ((sc != 0) && (score_>SCOREMIN) && (score_<SCOREMAX) && (identities!=IDENTITY) && (expect+0<EVALUE) &&((substr(query,sc,1) ~ PAT)))
		 {
		     	     query_Sec=query; # Keep Sec alignment
			     middle_line_Sec=middle_line;
			     sbjct_align_Sec=sbjct_align;
