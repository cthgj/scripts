...
BEGIN{
    ...
    bit=0;
    MATCH="C"; ## aa in subject that PAT should match
}
...
# Sec alignment (C-* or whatever we are interested)
    ...
    if ((identities >= IDENTITY) && (sc != 0) && (bit==0) && (score_>SCOREMIN) && (score_<SCOREMAX) && (expect+0<EVALUE) &&((substr(query,sc,1) ~ PAT)) && (sbjct_ID != query_ID_Sec)) ## final check to discard matches against self (usefull when blasting ESTs against est)
       { print hello;
...
# Special case when an *-* alignment follows another *-* in different HSPs
	     ...
	     else if ((identities >= IDENTITY) && (sc != 0) && (score_>SCOREMIN) && (score_<SCOREMAX) && (expect+0<EVALUE) &&((substr(query,sc,1) ~ PAT)) && (sbjct_ID!=query_ID_Sec))
		 {
		     	     query_Sec=query; # Keep Sec alignment
			     middle_line_Sec=middle_line;
			     sbjct_align_Sec=sbjct_align;
