...
if($opts{p}){
	...
	for (my $k=$i; $k<scalar(@GOs);$k++) {
	    ...
	    my $pair = join("_", @a);
	    ##
	    next unless $pair eq 'GO:0044459_GO:0044459';
	    ##
		...
		foreach my $go2prot (@{$inter_lists{$go1prot}}){
		    ...
		    #else{print "$go1prot : $prots{GOs}{$go1prot}{$go1} $go2prot : $prots{GOs}{$go2prot}{$go2}\n";}
		}
	    }
	    
	    $go_counts{GOs}{$a[0]}{$oo}=0 unless defined($go_counts{GOs}{$a[0]}{$oo});
	    $go_counts{GOs}{$a[1]}{$oo}=0 unless defined($go_counts{GOs}{$a[1]}{$oo});
	    print "$oo $pair\t$go_counts{ONTOs}{$oo}\t$go_counts{GOs}{$a[0]}{$oo}\t$go_counts{GOs}{$a[1]}{$oo}\t$both\t$same\n";
	}
    }
...
sub load_network {
  ...
  while (<$A>) {
    ...
    if ($mode==0) {
      ...
      if(defined($prots{GOs}{$bait}{'GO:0044459'}) ||
	 defined($prots{GOs}{$target}{'GO:0044459'}) ){
	  print "a $_ : $prots{$bait}{ONTOs}{C}\n";
      }
	  ...
	  map{
	      ...
	      if(/GO:0044459/ && $oo eq 'CC'){print "aa $oo $bait $target \$go_stats{ONTOs}{$oo} : $go_stats{ONTOs}{$oo}\n";
	  }
	  }keys(%k);	  
      }
    }
