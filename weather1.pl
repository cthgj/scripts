...
#	    if( $tmf && $hmt && $wnd && $sunset && $sunrise ){
	...
	while(<FILE>){
		...
		for(my $nn=0;$nn<scalar(@temps);$nn+=2){
		    if(length($temps[$nn])<2){$temps[$nn]=" " . $temps[$nn]}
		    print "     $temps[$nn] ";
		}
		print "\n   ";
...
sub cond_symb { #translates conditions into symbol in weather font
    ...
    # elsif ($_ =~ /Shower/){$_="h"; }
    if($night && $_ =~/Clear/){$_="~/conkyimages/moon.gif"}
    elsif($night && $_ =~/Fair/){$_="~/conkyimages/mclouds1.gif"}
    elsif($night && $_ =~/Cloudy/){$_="~/conkyimages/mclouds2.gif"}
    elsif ($_ =~ "Partly Cloudy"){$_="~/conkyimages/partly_cloudy.gif";}
    elsif ($_ =~ /Wind/ ){$_="~/conkyimages/wind.gif";}
    elsif($_ =~ /Mostly\sSunny/ || $_ =~ /Mostly\sClear/ ){$_="~/conkyimages/mostly_sunny.gif"}
    elsif ($_ =~ /Mostly\sCloudy/){$_="~/conkyimages/mostly_cloudy.gif";}
    elsif ($_ =~ "Cloud" || $_ =~ "Fog"){$_="~/conkyimages/cloudy.gif";}
    elsif ($_ =~ "Storm" || $_ =~ "Thunder" || $_ =~ "T-"){$_="~/conkyimages/storm.gif";}
    elsif ($_ =~ "Snow" || $_ =~ "Flurries" || $_ =~ "Wintry"){$_="~/conkyimages/snow.gif";}
    #elsif(/Shower/ && /Wind/){$_="f"}
    elsif ($_ =~ "Fair"  || $_ =~ "Sunny"|| $_ =~ "Sun" || $_ =~ "Clear"){$_="~/conkyimages/sunny.gif";}
    elsif(/AM\sShowers/){$_="~/conkyimages/am_showers.gif"}
    elsif(/Scattered\sShowers/){$_="~/conkyimages/showers.gif"}
    elsif ($_ =~ /Rain/ || /Drizzle/){$_="~/conkyimages/rain.gif";}
    elsif ($_ =~ /Showers/){$_="~/conkyimages/showers4.gif"; }
